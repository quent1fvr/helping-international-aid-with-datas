...
df = pd.read_csv("Donnees_projet_2021/pathbased.txt") # import des données

head = str(df.columns[0]).split('\t', 3)
df = df["11.25	5.05	1"].str.split('\t',3, expand = True)
...
kmeans = sc.KMeans(n_clusters=3, init='k-means++', n_init=10).fit(X)
...
gm = GaussianMixture(n_components = 3, covariance_type= 'full', n_init=10).fit(Z)
...
dbscan = sc.DBSCAN(eps = 0.5, min_samples = 3,algorithm='auto').fit(Z)
...
spect = sc.SpectralClustering(n_clusters = 3, n_init = 10, affinity = 'rbf', n_neighbors=10).fit(Z)
